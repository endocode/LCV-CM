...
```
to avoid docker build from using cache. The dockerfile clone this repository, so if it is required to update the docker image with code added recently, `--no-cache` would be the right option to apply the changes.
